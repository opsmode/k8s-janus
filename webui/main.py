...
#
#   1. SessionMiddleware         — in-memory sessions (uses sticky sessions for multi-replica)
...
@app.get("/mfa-verify", include_in_schema=False, response_class=HTMLResponse)
...
# ---------------------------------------------------------------------------
# Profiles are stored in the database when available, otherwise in-memory.
# Multi-replica deployments should enable PostgreSQL to share profile state.

_memory_profiles: dict[str, dict] = {}  # Fallback when DB disabled
...
@app.get("/api/mfa/status", include_in_schema=False)
...
@app.post("/api/mfa/setup", include_in_schema=False)
...
@app.post("/api/mfa/enable", include_in_schema=False)
...
@app.post("/api/mfa/disable", include_in_schema=False)
...
@app.post("/api/mfa/verify", include_in_schema=False)
...
@app.post("/api/mfa/backup-codes", include_in_schema=False)
